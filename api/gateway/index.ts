import { User, Users, LoginId, Recommends, CandidateId, RecommendMessage, Recommend, UserId, Candidates, Candidate } from "../domain";
...
export class CandidatesGateway extends CandidatesPort {
  ...
  async list(): Promise<Candidates> {
    ...
    const usersEntity = await Promise.all(recommendsEntity.map(it => this.usersDriver.findById(it.recommenderId)));
    const users = new Users(usersEntity.map(User.from));
    const recommends = recommendsEntity.map(it => ({
      candidateId: it.candidateId,
      recommend: new Recommend(users.findBy(new UserId(it.recommenderId)), new RecommendMessage(it.recommend))
    }));
    const mappedRecommends = recommends.reduce<{ [key: number]: Recommend[] }>((prev, curr) => {
    ...
    const candidateArray = Object.keys(mappedRecommends).map(it => new Candidate(new CandidateId(parseInt(it, 10)), new Recommends(mappedRecommends[it])));
    return new Candidates(candidateArray);
  }

  async create(id: CandidateId, recommenderId: LoginId, message: RecommendMessage): Promise<void> {
