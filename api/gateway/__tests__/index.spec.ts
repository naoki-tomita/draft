...
import { Users, User, LoginId, UserId, Candidates, Candidate, CandidateId, Recommends, Recommend, RecommendMessage } from "../../domain";
...
describe("UsersGateway", () => {
  ...
  describe("create", () => {
    it("should create User and returns the User.", async () => {
      ...
      when(findByLoginId).calledWith("loginId").mockReturnValueOnce({ id: 1, loginId: "loginId" });
  ...
  describe("findByLoginId", () => {
    it("should find user.", async () => {
      ...
      when(findByLoginId).calledWith("id").mockReturnValue({ id: 1, loginId: "id" });
    ...
    it("should not find user if user not found.", async () => {
      ...
      when(findByLoginId).calledWith("id").mockReturnValue(null);
...
describe("RecommendsGateway", () => {
  ...
  describe("list", () => {
    it("should return list", async () => {
      ...
      when(findAll).calledWith().mockReturnValueOnce(recommendEntities);
      when(findById).calledWith(10).mockReturnValueOnce(user10Entity);
      when(findById).calledWith(20).mockReturnValueOnce(user20Entity);
      when(findById).calledWith(30).mockReturnValueOnce(user30Entity);

      const expected = new Candidates([
        new Candidate(new CandidateId(0), new Recommends([
          new Recommend(new User(new UserId(10), new LoginId("foo")), new RecommendMessage("message10")),
          new Recommend(new User(new UserId(20), new LoginId("bar")), new RecommendMessage("message20")),
        ])),
        new Candidate(new CandidateId(1), new Recommends([
          new Recommend(new User(new UserId(30), new LoginId("hoge")), new RecommendMessage("message30")),
        ])),
      ]);
  ...
  describe("create", () => {
    it("should create", async () => {
      ...
      when(findByLoginId).calledWith(recommenderId.value).mockReturnValueOnce({ id: 10 });
      await candidatesGateway.create(id, recommenderId, message);
      when(create).expectCalledWith(id.value, recommenderId.value, message.value);
    });
  });
});
